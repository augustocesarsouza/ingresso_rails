module ScheduleTimeBuyHelper
  def render_time_buy_container(schedule_parts, movie)
    puts movie.inspect
    ...
    schedule_parts.each do |schedule_part|
      ...
      schedule_part_split_spaces.each do |schedule_part_split_space|
        if schedule_part_split_space == "d"
            rendered_content += content_tag(:div, class: "container-svg-ticket", data: { route: chose_seats_and_more_index_path(id: movie.id) }) do
  ...
  def render_time_buy_vip_dublado_container(schedule_parts, movie)
    rendered_content = ''.html_safe
  ...
  def render_time_buy_vip_3d_dublado_container(schedule_parts, movie)
    rendered_content = ''.html_safe
  ...
  def render_time_buy_3d_dbox_dublado_container(schedule_parts, movie)
    rendered_content = ''.html_safe
  ...
  def render_time_buy_telasgigantes_dublado_container(schedule_parts, movie)
    rendered_content = ''.html_safe
